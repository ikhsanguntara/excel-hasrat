...
/**
 * Generator Laporan Checkpoint & Inspeksi Khusus Format YAMAHA
 */
async function generateYamahaExcel(items) {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Laporan Checkpoint Yamaha', {
        views: [{ showGridLines: true }]
    });

    const FONT_FAMILY = 'Segoe UI';

    // Palet Warna Khas Yamaha & Slate Executive
    ...
    // --- 1. BANNER JUDUL YAMAHA ---
    worksheet.mergeCells('A1:F1');
    const titleCell = worksheet.getCell('A1');
    titleCell.value = '  LAPORAN HASIL CHECKPOINT & INSPEKSI (YAMAHA)';
    titleCell.font = { name: FONT_FAMILY, size: 16, bold: true, color: { argb: 'FFFFFFFF' } };
    ...
    const headers = [
        ...
        { header: 'Kode / Mandat', key: 'code', width: 15 },
    ...
    for (const [areaName, sections] of Object.entries(groupedData)) {
        ...
        for (let c = 1; c <= 6; c++) worksheet.getCell(currentRow, c).border = borderCell;
        currentRow++;
        ...
        for (const [sectionName, secItems] of Object.entries(sections)) {
            ...
            for (const item of secItems) {
                ...
                const seqNum = item.seq_num || rowCounter;
                const isMandatory = item.is_mandatory === 'Y' ? ' (Wajib)' : '';
                const codeStr = `#${seqNum}${isMandatory}`;
                ...
                const c2 = worksheet.getCell(currentRow, 2); c2.value = codeStr; c2.alignment = alignCenter;
                const c3 = worksheet.getCell(currentRow, 3); c3.value = cpName; c3.alignment = alignLeft;
                const c4 = worksheet.getCell(currentRow, 4); c4.value = resultVal; c4.alignment = alignCenter;
                const c5 = worksheet.getCell(currentRow, 5); c5.value = secDate; c5.alignment = alignCenter;
                const c6 = worksheet.getCell(currentRow, 6); c6.alignment = alignCenter;

                // Color Result Badge
                if (resultVal.toUpperCase() === 'Y') {
                    c4.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFDCFCE7' } };
                    c4.font = { name: FONT_FAMILY, size: 10, bold: true, color: { argb: 'FF15803D' } };
                } else if (resultVal.toUpperCase() === 'N') {
                    c4.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFEE2E2' } };
                    c4.font = { name: FONT_FAMILY, size: 10, bold: true, color: { argb: 'FFB91C1C' } };
                } else {
                    c4.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFE0F2FE' } };
                    c4.font = { name: FONT_FAMILY, size: 10, bold: true, color: { argb: 'FF0369A1' } };
                }
                ...
                if (imgData) {
                    ...
                    worksheet.addImage(imageId, {
                        tl: { col: 5.08, row: currentRow - 0.9 },
